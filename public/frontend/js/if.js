...
("use strict");
const iframeVideoPlugin = {
    // Iframe Video On Scroll
    init() {
        this.videoEls = Array.from(
        ...
        if (this.videoEls.length > 0) {
            Array.prototype.forEach.call(this.videoEls, (videoEl) => {
                if (videoEl.src.search(this.notSupportedVideosRegex) > 0) {
                    ...
                    if (
                        videoEl.src.search("twitch.tv") > 0 &&
                        videoEl.src.search("autoplay=false") < 0
                    ) {
                        // twitch exception
                    ...
                    videoEl.src = videoEl.src.replace(
                        this.autoStartRegex,
                        "autostart=false"
                    );
                    videoEl.onload = () => {
    ...
    notSupportedVideosClickDetect(videoEl) {
        ...
        window.addEventListener(
            "blur",
            this.notSupportedVideosWindowBlurHandler,
            false
        );
        window.focus();
    },
    ...
    isInViewPort(videoEl) {
        let percentVisible = this.fraction,
            ...
            isVisible =
                elemTop >= -overhang &&
                elemBottom <= window.innerHeight + overhang;

        if (isVisible) {
            // video is in the ViewPort, play it
            if (
                !videoEl.classList.contains("playing") &&
                videoEl.src.search(this.notSupportedVideosRegex) < 0
            ) {
                videoEl.classList.add("playing");
                if (videoEl.classList.contains("rhd-auto-play")) {
                    videoEl.contentWindow.postMessage(
                        this.messageFn("play", videoEl.src),
                        "*"
                    );
                }
            }
            ...
            if (videoEl.classList.contains("playing")) {
                if (videoEl.src.search(this.notSupportedVideosRegex) > 0) {
                ...
                } else if (videoEl.classList.contains("rhd-twitter-video")) {
                    // stop only twitter videos
                    videoEl.classList.remove("playing");
                    let isVideoIframeInIframe =
                        videoEl.contentDocument.querySelector("iframe");
                    if (isVideoIframeInIframe) {
                        // if twitter video is playing
                    ...
                    videoEl.classList.remove("playing");
                    videoEl.contentWindow.postMessage(
                        this.messageFn("pause", videoEl.src),
                        "*"
                    );
                }
            }
...
const loadJS = (source, callback) => {
    // Fn to load JS files and append to <head></head>
    ...
    scriptEl.onload = scriptEl.onreadystatechange = function (_, isAbort) {
        if (
            isAbort ||
            !scriptEl.readyState ||
            /loaded|complete/.test(scriptEl.readyState)
        ) {
...
const checkTwitterEmbed = () => {
    let twitterEls = document.querySelectorAll(
        ".twitter-video, .twitter-tweet"
    );
